...
#读取测试用例
test_data = Do_Excel(project_path.case_path,'carInfo').read_data('CarInfoCaseId')
log = Log() #日志记录
...
class Test_Login(unittest.TestCase):#模块、类、函数名都要test开头，否则不会执行 这是python默认规则，也可以自己修改规则，自行百度扩展
    ...
    def setUp(self) : #相关于预置条件
        ...
        try:
            ...
            self.t = Do_Excel(project_path.case_path,'carInfo') #sheetname忘记改了
    ...
    def test_login(self,case):#使用case 作为接收参数数据的变量名
        ...
        try:
            ...
            raise e
        # print(resp.json())
        # print(eval(case['ExpectedResult']))
        if case['CaseId']!=1: #判断不是1的时候,执行下面的代码
            #断言并讲测试结果写回excel中
            print(resp.json())
            print(eval(case['ExpectedResult']))
            try:
                self.assertEqual(resp.json()['message'],eval(case['ExpectedResult'])['message']) #判断 实际结果和预期结果是否相等
                ...
                log.error('{}模块第{}条用例的返回测试结果:{}'.format(case['Module'], case['CaseId'],e))
                raise e
